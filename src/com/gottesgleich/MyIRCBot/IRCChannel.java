...
public class IRCChannel {
	...
	public void leave() {
		this.con.getOutput().addMessage(
				new IrcProtocol(Action.LEAVE, "", this.name));
		this.con.getChannels().remove(this);
		if (this.con.getActiveChannel().equals(this)) {
			this.con.setActiveChannel(this.con.getChannels().size() != 0 ? this.con
					.getChannels().get(0) : null);
		}
	}

	public void sendMessageToChannel(String msg) {
		this.con.getOutput().addMessage(
				new IrcProtocol(Action.MSG, msg, this.name));
	}

	public void sendRAWMessageToChannel(String msg) {
		this.getConnection().getOutput()
				.addMessage(new IrcProtocol(Action.RAW, msg, this.name));
	}

	public void updateClientList() {
		this.getConnection().getOutput()
				.addMessage(new IrcProtocol(Action.NAMES, "", this.name));
	}
	...
	public void kickClient(String client, String reason) {
		this.getConnection()
				.getOutput()
				.addMessage(
						new IrcProtocol(Action.KICK, client + "::" + reason,
								this.name));
	}
	...
	public void setMode(String user, String mode) {
		this.getConnection()
				.getOutput()
				.addMessage(
						new IrcProtocol(Action.MODE, user + "::" + mode,
								this.name));
	}

	public void setTopic(String topic) {
		this.getConnection().getOutput()
				.addMessage(new IrcProtocol(Action.TOPIC, topic, this.name));
	}
